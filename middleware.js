...
export function middleware(req) {
  //const lng = 'en'; // Set your default language
  ...
  // Allow requests for image files
  if (
    req.nextUrl.pathname.startsWith('/_next') ||
    req.nextUrl.pathname.match(/\.(png|jpg|jpeg|gif|svg|webp)$/) // Regex to match image file extensions
  ) {
    return NextResponse.next();
  }

  // Redirect logic for languages
  if (
    !languages.some(loc => req.nextUrl.pathname.startsWith(`/${loc}`))
  ) {
